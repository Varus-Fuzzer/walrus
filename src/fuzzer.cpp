...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t* Data, size_t Size)
{
    ...
    auto engine = std::make_unique<Walrus::Engine>();
    auto store = std::make_unique<Walrus::Store>(engine.get());

    Walrus::DefinedFunctionTypes functionTypes;

    Trap::TrapResult trapResult = executeWASM(store.get(), filename, input, functionTypes);
